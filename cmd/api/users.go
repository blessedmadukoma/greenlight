...
func (app *application) registerUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// add "movie:read" permission to the user
...
func (app *application) updateUserPasswordHandler(w http.ResponseWriter, r *http.Request) {
	var input struct {
		Password      string `json:"password"`
		TokenPlantext string `json:"token"`
	}
	...
	data.ValidateTokenPlaintext(v, input.TokenPlantext)
	...
	user, err := app.models.Users.GetForToken(data.ScopePasswordRest, input.TokenPlantext)
	if err != nil {
		switch {
		case errors.Is(err, data.ErrRecordNotFound):
	...
	err = app.models.Tokens.DeleteAllForUser(data.ScopePasswordRest, user.ID)
	if err != nil {
		app.serverErrorResponse(w, r, err)
		return
	}
